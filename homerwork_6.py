    ...
    for i in path.iterdir():
      ...
      if i.is_dir() and i.name not in extensions.keys():
        ...
        if f_type == 'archives':
                ...
                i = i.replace(str(path)+'\\'+name)
                if suffix == 'RAR':
                    rf = rarfile.RarFile(str(path)+'\\'+name)
                    rf.extractall(path = str(path)+'\\'+f_type+'\\'+i.stem+'\\' )
                else:
...
rarfile.UNRAR_TOOL = r"C:\Program Files\WinRAR\UNrar.exe" # указать путь к разархиватору на машине, где исполняется код
parse_folder(Path(parse_args()))
#parse_folder(Path('C:/test — копия/'))
